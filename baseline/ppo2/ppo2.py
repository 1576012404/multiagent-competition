...
from baseline.common import explained_variance, set_global_seeds
from baseline.common.policies import build_policy
try:
    from mpi4py import MPI
except ImportError:
    MPI = None
from baseline.ppo2.runner import Runner
...
def learn(*, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baseline.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if model_fn is None:
        from baseline.ppo2.model import Model
        model_fn = Model
    ...
    if load_path is not None:
        for i_load_path in load_path:
            model.load(i_load_path)
    # Instantiate the runner object
    runner = Runner(env=env, model_list=model_list, nsteps=nsteps, gamma=gamma, lam=lam)
            ...
            for i in range(num_agent):
                for _ in range(noptepochs):
                    ...
                    for start in range(0, nbatch, nbatch_train):
                        ...
                        mblossvals[i].append(model_list[i].train(lrnow, cliprangenow, *slices))
